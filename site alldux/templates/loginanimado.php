...
<script>
      ...
      const mensagemRetorno = document.getElementById('mensagem_retorno');
      ...
      container.addEventListener('click', async () => {
            ...
            try {
                  ...
                  // O tempo para exibir a mensagem e redirecionar
                  const delay = 2500; // 2.5 segundos, para a animação ter tempo de rodar

                  if (result.success) {
                        // Cadastro bem-sucedido
                        mensagemRetorno.textContent = result.message;
                        mensagemRetorno.classList.remove('error');
                        mensagemRetorno.classList.add('success');

                        // Aguarda o fim da animação de sucesso antes de redirecionar
                        setTimeout(() => {
                        ...
                        mensagemRetorno.classList.add('error');
                        container.classList.add('error');

                        setTimeout(() => {
                              container.classList.remove('error');
                              isProcessing = false; // Permite um novo clique após a animação de erro
                        }, 1000);
                  }
                  ...
                  container.classList.remove('active');
                  mensagemRetorno.textContent = "Erro de conexão. Tente novamente.";
                  mensagemRetorno.classList.remove('success');
                  mensagemRetorno.classList.add('error');
                  container.classList.add('error');
