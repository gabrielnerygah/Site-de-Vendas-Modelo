...
$host = '127.0.0.1:3306';
$db = 'u393630075_database';
$user = 'u393630075_admin';
$pass = 'Gdml22052007$';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['imagem']) && $_FILES['imagem']['error'] == 0) {
        ...
        if (move_uploaded_file($imagem['tmp_name'], $caminhoImagem)) {
            ...
            $stmt->bind_param("ssdsi", $nome, $descricao, $preco, $caminhoImagem, $usuarioId);
            $stmt->execute();
        } else {
...
<nav class="menu-lateral">
    ...
    <ul>
        ...
        <li class="item-menu">
            <a href="#">
                <button onclick="redirecionaDashboard()">
...
<?php endif; ?>
    ...
    <div class="container">
        <div class="container2">    <h3>Adicionar Produto</h3>
            ...
            <?php while ($produto = $result->fetch_assoc()): ?>
                <div class="product">
                    <?php if ($produto['imagem']): ?>
                        <!-- Exibir imagem usando o caminho salvo no banco de dados -->
                        <img src="<?= htmlspecialchars($produto['imagem']) ?>" alt="Imagem do Produto">
                    <?php else: ?>
                        <img src="default.png" alt="Imagem Padrão">
                    <?php endif; ?>
                    <div class="product-info">
                        <strong><?= htmlspecialchars($produto['nome']) ?></strong><br>
                        <span><?= htmlspecialchars($produto['descricao']) ?></span><br>
                        <span>R$ <?= number_format($produto['preco'], 2, ',', '.') ?></span><br>
                    </div>
                </div>
            <?php endwhile; ?>
        </div>
